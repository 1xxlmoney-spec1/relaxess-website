...
const STORAGE_KEYS = {
  MESSAGES_TODAY: '@calmspace_messages_today',
  LAST_MESSAGE_DATE: '@calmspace_last_message_date',
  DAILY_LIMIT: '@calmspace_daily_limit',
};
...
export function OpenAIProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const initializeProvider = async () => {
      try {
        ...
        console.log('[OpenAI Context] 🔍 Checking for API key...');
        console.log('[OpenAI Context] process.env keys with OPENAI:', Object.keys(process.env).filter(k => k.includes('OPENAI')));
        console.log('[OpenAI Context] Constants.expoConfig?.extra:', Constants.expoConfig?.extra);
        ...
        console.log('[OpenAI Context] ✓ OpenAI API key loaded successfully');
        console.log('[OpenAI Context] 🔑 API key (first 20 chars):', apiKey.substring(0, 20) + '...');
        ...
        if (lastMessageDate === today) {
          ...
          console.log('✓ Loading message count from today');
          ...
          // New day - reset count
          console.log('✓ New day detected - resetting message count');
  ...
  useEffect(() => {
    if (appSession.isPremium) {
      ...
      console.log('✓ Premium user detected - unlimited messages enabled');
    } else {
      // Free users have 10 messages per day
      setDailyMessageLimitState(10);
      console.log('✓ Free user - 10 messages per day limit applied');
    }
    ...
    async (content: string) => {
      ...
      try {
        ...
        if (!response.success) {
          ...
          console.warn('OpenAI API error:', response.error);
